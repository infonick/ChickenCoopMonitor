...
<main class='container py-5' id='mainSection'>
    ...
    <section class='row'>
        <div class="mx-auto">
            Chicken Door Override: 
			<button type="button" class="btn btn-primary" id='activateChickenDoorOverride'>Activate</button> 
			<button type="button" class="btn btn-primary" id='deactivateChickenDoorOverride'>Deactivate</button>
        </div>
...
$(document).ready(function(){
	...
	$('#activateChickenDoorOverride').click(function() {
        //console.log("starting..");
        var data = {page: "Main", command: "chickenDoorOpen"};
	...
	$('#deactivateChickenDoorOverride').click(function() {
        //console.log("starting..");
        var data = {page: "Main", command: "chickenDoorAuto"};
        var url = 'controller.php';
        $.post(url, data, function(data) {
            //console.log(data);
        });
    ...
    function sensorTable(data){
        console.log(data);
		var overrideStatus = "";
		var chickenDoorStatus = "";

		try{
			data = JSON.parse(data);
			overrideStatus = data['Doorkeeper Override'];
			chickenDoorStatus = data['Chicken Door'];
		}
		catch (err){
			overrideStatus = "Error";
			chickenDoorStatus = "Error";
		}
		
		var htmlOut = "<div class='row'><table class='table'>";
		htmlOut += "<tr><th>Chicken Door Status</th><th>" + chickenDoorStatus + "</th></tr>";
		htmlOut += "<tr><th>Doorkeeper Override</th><th>" + overrideStatus + "</th></tr>";
		htmlOut += "</table></div>";

        $('#dataDiv').html(htmlOut);
